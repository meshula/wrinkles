...
struct OrdinateImpl {
    ...
    OrdinateType sqrt() const noexcept { return OrdinateType{ std::sqrt(v) }; }
    ...
    // 1. If ExpT is floating point, we use std::pow.
    ...
    OrdinateType pow(ExpT exp) const noexcept {
        // 1. If the exponent is a floating-point type, use std::pow.
        if constexpr (std::is_floating_point_v<ExpT>) {
            using std::pow;
            return OrdinateType{ pow(v, exp) };
        }
        // 2. Otherwise, check if the underlying type T has a member 'pow' 
        // that can be called with the exponent 'exp'.
        ...
        // 3. Otherwise, static_assert to fail compilation.
             ...
             // Return T(0) as a fallback in the unlikely case static_assert is disabled
...
inline bool eql_approx(A const& a, B const& b) {
    if constexpr (requires { a.eql_approx(b); }) return a.eql_approx(b);
        ...
        using V = decltype(a);
        using Numeric = std::conditional_t<std::is_arithmetic_v<V>, V, double>;
