...
HodoBezierSegment compute_hodograph(const HodoBezierSegment* const b)
{
    HodoBezierSegment r = {0, {{0,0}, {0,0}, {0,0}, {0,0}}};
...
Vector2 bezier_roots(const HodoBezierSegment* const bz) {
    Vector2 rv = { -1.f, -1.f };
    ...
    if (bz->order == 2) {
        ...
        (void)m; // Calculate for potential future use, suppress unused warning
...
// is on the x-axis
HodoBezierSegment align_bezier(const HodoBezierSegment* const bz) {
    if (!bz || bz->order < 2 || bz->order > 3) {
        return (HodoBezierSegment) {0, {{0,0}, {0,0}, {0,0}, {0,0}}};
    }

    if (bz->order == 3) {
        HodoBezierSegment rv;
        rv.order = 3;
        rv.p[0] = (Vector2) {0, 0};
...
Vector2 inflection_points(const HodoBezierSegment* const bz) {
    if (!bz || bz->order != 3)
        return (Vector2){-1.f, -1.f};
    
    /// @TODO for order 2

    HodoBezierSegment aligned = align_bezier(bz);
...
bool split_bezier(const HodoBezierSegment* bz, float t, HodoBezierSegment* r1, HodoBezierSegment* r2)
{
...
Vector2 evaluate_bezier(HodoBezierSegment* b, float u)
{
    Vector2 r = {0, 0};
    if (!b || b->order < 2 || b->order > 3)
        return r;
