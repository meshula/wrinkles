...
pub fn value_at_time_between(t: f32, fst: ControlPoint, snd: ControlPoint) f32 {
...
pub fn _bezier0(unorm: f32, p2: f32, p3: f32, p4: f32) f32 {
    ...
    return (p4 * z3) - (p3 * (3.0 * z2 * zmo)) + (p2 * (3.0 * z * zmo2)) - (p1 * zmo3);
}
...
pub fn normalized_to(
    ...
    for (result.segments, 0..) |seg, seg_index| {
        ...
        for (seg.points(), 0..) |pt, pt_index| {
            new_points[pt_index] = .{
                .time = remap_float(pt.time, crv_min.time, crv_max.time, min_point.time, max_point.time),
                .value = remap_float(pt.value, crv_min.value, crv_max.value, min_point.value, max_point.value),
            };
        }
...
test "remap_float" {
    try expectEqual(remap_float(0.5, 0.25, 1.25, -4, -5), @as(f32, -4.25));
}

test "normalized_to" {
    var slope2 = [_]curve.Segment{curve.create_bezier_segment(
        .{ .time = -500, .value = 600 },
        .{ .time = -300, .value = -100 },
        .{ .time = 200, .value = 300 },
        .{ .time = 500, .value = 700 },
...
test "normalize_to_screen_coords" {
    var segments = [_]curve.Segment{curve.create_bezier_segment(
        .{ .time = -500, .value = 600 },
        .{ .time = -300, .value = -100 },
        .{ .time = 200, .value = 300 },
        .{ .time = 500, .value = 700 },
    )};
    const input_crv: curve.TimeCurve = .{ .segments = &segments };
...
pub fn inverted_linear(crv: linear_curve.TimeCurveLinear) linear_curve.TimeCurveLinear {
    ...
    var result = crv;
    result.knots = ALLOCATOR.dupe(ControlPoint, crv.knots) catch unreachable;
    ...
    for (crv.knots, 0..) |knot, index| {
        ...
        const target_index = (if (needs_reversal) knot_count - real_index - 1 else real_index);
...
pub fn inverted_bezier(crv: curve.TimeCurve) linear_curve.TimeCurveLinear {
...
test "inverted: invert linear" {
    ...
    // slope of 2
    curve.create_linear_segment(.{ .time = -1, .value = -3 }, .{ .time = 1, .value = 1 })};
    ...
    {
        errdefer std.log.err("knot0.time ({any}) < knot1.time ({any})", .{ inverse_crv.knots[0].time, inverse_crv.knots[1].time });
    ...
    while (t < 1) : (t += 0.01) {
        ...
        errdefer std.log.err("[t: {any}] ident: {any} forwd: {any} inv: {any}", .{ t, idntity_p, forward_p, inverse_p });
...
fn _rescale_val(t: f32, measure_min: f32, measure_max: f32, target_min: f32, target_max: f32) f32 {
...
fn _rescaled_pt(
    ...
    return .{
        .time = _rescale_val(pt.time, extents[0].time, extents[1].time, target_range[0].time, target_range[1].time),
        .value = _rescale_val(pt.value, extents[0].value, extents[1].value, target_range[0].value, target_range[1].value),
    };
}
...
test "TimeCurve: rescaled parameter" {
    ...
    const result = try rescaled_curve(crv, .{
        .{ .time = 100, .value = 0 },
        .{ .time = 200, .value = 10 },
    });
