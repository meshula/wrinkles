...
pub const TimeCurveLinear = struct {
    ...
    pub fn init(knots: []const ControlPoint) error{OutOfMemory}!TimeCurveLinear {
        return TimeCurveLinear{ .knots = try ALLOCATOR.dupe(ControlPoint, knots) };
    }
    ...
    pub fn evaluate(self: @This(), t_arg: f32) error{OutOfBounds}!f32 {
        if (self.nearest_smaller_knot_index(t_arg)) |index| {
    ...
    pub fn nearest_smaller_knot_index_value(self: @This(), v_arg: f32) []u32 {
        const bounds = self.extents();
        if (self.knots.len == 0 or v_arg < bounds[0].value or v_arg >= bounds[1].value) {
            return .{};
        }
        ...
        for (self.knots[0 .. self.knots.len - 1], 0..) |knot, index| {
            const next_knot = self.knots[index + 1];
            if ((knot.value <= v_arg and v_arg < next_knot.value) or (knot.value >= v_arg and v_arg > next_knot.value)) {
                result.append(index);
            }
        }
    ...
    pub fn nearest_smaller_knot_index(
        ...
        if (self.knots.len == 0 or (t_arg < self.knots[0].time) or t_arg >= self.knots[self.knots.len - 1].time) {
            return null;
        }
    ...
    ///
    pub fn project_curve( // curve being projected _through_
        self: @This(), // curve being projected
        ...
        for (self.knots) |self_knot| {
            if (_is_between(self_knot.time, other_bounds[0].value, other_bounds[1].value)) {
        ...
        for (other_copy.knots, 0..) |other_knot, index| {
            if (self_bounds[0].time <= other_knot.value and other_knot.value <= self_bounds[1].time) {
                if (index != last_index + 1) {
                    ...
                    if (current_curve.items.len > 1) {
                        curves_to_project.append(TimeCurveLinear.init(current_curve.items) catch unreachable) catch unreachable;
                    }
        ...
        for (curves_to_project.items) |crv| {
            // project each knot
            for (crv.knots, 0..) |knot, index| {
                ...
                //    check in that case
                const value = self.evaluate(knot.value) catch (if (self.knots[self.knots.len - 1].time == knot.value)
