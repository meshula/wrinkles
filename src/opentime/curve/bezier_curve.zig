...
pub fn distance(from: ControlPoint, to: ControlPoint) f32 {
...
test "distance: 345 triangle" {
    try expectEqual(@as(f32, 5), distance(.{ .time = 3, .value = -3 }, .{ .time = 6, .value = 1 }));
}
...
pub const Segment = struct {
    ...
    pub fn eval_at(self: @This(), unorm: f32) ControlPoint {
    ...
    pub fn can_project(self: @This(), segment_to_project: Segment) bool {
        ...
        // _ = snd_test;
        //
        return (other_extents[0].value >= my_extents[0].time - generic_curve.EPSILON and other_extents[1].value < my_extents[1].time + generic_curve.EPSILON);
    }

    /// assuming that segment_to_project is contained by self, project the
    /// points of segment_to_project through self
    pub fn project_segment(self: @This(), segment_to_project: Segment) Segment {
    ...
    pub fn eval_at_x(self: @This(), x: f32) f32 {
        const u: f32 = bezier_math.findU(x, self.p0.time, self.p1.time, self.p2.time, self.p3.time);
    ...
    pub fn debug_print_json(self: @This()) void {
        std.debug.print("\ndebug_print_json] p0: {}\n", .{self.p0});
...
fn _is_approximately_linear(segment: Segment, tolerance: f32) bool {
...
pub fn linearize_segment(
    ...
    //        bezier segments
    var result: std.ArrayList(ControlPoint) = (std.ArrayList(ControlPoint).init(ALLOCATOR));
...
test "segment from point array" {
    const original_knots_ident: [4]ControlPoint = .{ .{ .time = -0.5, .value = -0.5 }, .{ .time = -0.16666, .value = -0.16666 }, .{ .time = 0.166666, .value = 0.16666 }, .{ .time = 0.5, .value = 0.5 } };
    ...
    try expectApproxEql(original_knots_ident[3].time, linearized_ident_knots.items[1].time);
    try expectApproxEql(original_knots_ident[3].value, linearized_ident_knots.items[1].value);
}
...
pub fn create_linear_segment(p0: ControlPoint, p1: ControlPoint) Segment {
    ...
    debug_panic("Create bezier segment failed, {}, {}\n", .{ p0.time, p1.time });
}

pub fn create_identity_segment(t0: f32, t1: f32) Segment {
    return create_linear_segment(.{ .time = t0, .value = t0 }, .{ .time = t1, .value = t1 });
}
...
pub fn create_bezier_segment(p0: ControlPoint, p1: ControlPoint, p2: ControlPoint, p3: ControlPoint) Segment {
...
pub const TimeCurve = struct {
    ...
    pub fn init(segments: []const Segment) error{OutOfMemory}!TimeCurve {
    ...
    pub fn init_from_linear_curve(crv: linear_curve.TimeCurveLinear) TimeCurve {
        ...
        for (crv.knots[0 .. crv.knots.len - 1], 0..) |knot, index| {
    ...
    pub fn insert(self: @This(), seg: Segment) void {
        ...
        std.sort(Segment, self.segments.to_slice(), generic_curve.cmpSegmentsByStart);
    }
    ...
    pub fn linearized(self: @This()) linear_curve.TimeCurveLinear {
        ...
        for (self.segments) |seg| {
            ...
            linearized_knots.appendSlice((linearize_segment(seg, 0.000001) catch unreachable).items) catch unreachable;
        }

        return .{ .knots = linearized_knots.items };
    }
...
test "TimeCurve: positive length 1 linear segment test" {
    const xform_curve: TimeCurve = .{ .segments = &[1]Segment{create_linear_segment(
        .{
            .time = 1,
            .value = 0,
        },
...
test "TimeCurve: projection_test - compose to identity" {
    var seg_0_4 = [_]Segment{create_linear_segment(
        .{
            .time = 0,
            .value = 0,
        },
    ...
    var seg_0_8 = [_]Segment{create_linear_segment(
        .{
            .time = 0,
            .value = 0,
        },
...
test "TimeCurve: projection_test non-overlapping" {
    ...
    var seg_1_9 = [_]Segment{create_linear_segment(
        .{
            .time = 1,
            .value = 1,
        },
...
test "positive slope 2 linear segment test" {
    var test_segment_arr = [_]Segment{create_linear_segment(
        .{
            .time = 1,
            .value = 0,
        },
...
test "negative length 1 linear segment test" {
    ...
    // by stack unwinding
    var segments_xform = [_]Segment{create_linear_segment(
        .{
            .time = -2,
            .value = 0,
        },
...
test "json writer: curve" {
    const ident = try TimeCurve.init(&.{create_identity_segment(-20, 30)});
