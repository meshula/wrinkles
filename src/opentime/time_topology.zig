...
const AffineTopology = struct {
    ...
    pub fn as_linear_curve_over_output_range(self: @This(), output_interval: interval.ContinuousTimeInterval) !curve.TimeCurveLinear {
        const self_inverted = self.transform.inverted();

        const other_bounds_in_input_space = self_inverted.applied_to_bounds(output_interval);

        const result_bounds = interval.intersect(other_bounds_in_input_space, self.bounds);

        if (result_bounds) |b| {
            const bound_points = [2]control_point.ControlPoint{
                .{ .time = b.start_seconds, .value = try self.project_ordinate(b.start_seconds) },
                .{ .time = b.end_seconds, .value = try self.project_ordinate(b.end_seconds) },
            };
    ...
    pub fn inverted(self: @This()) !TimeTopology {
        ...
        return .{
            .affine = .{ .bounds = .{ .start_seconds = start, .end_seconds = end }, .transform = self.transform.inverted() },
        };
    }
    ...
    pub fn project_topology(self: @This(), other: TimeTopology) TimeTopology {
        return switch (other) {
            .affine => |other_aff| {
                ...
                const self_bounds_in_input_space = (inv_xform.applied_to_bounds(self.bounds));
                ...
                if (bounds) |b| {
                    return .{
                        .affine = .{ .bounds = b, .transform = (self.transform.applied_to_transform(other_aff.transform)) },
                    };
                } else {
                    return .{ .empty = .{} };
                }
...
pub const BezierTopology = struct {
    ...
    pub fn project_topology(
        ...
        return switch (other) {
            .affine => |aff| {
                const aff_linearized = try aff.as_linear_curve_over_output_range(self.compute_bounds());

                const projected_curve = (self.bezier_curve.linearized().project_curve(aff_linearized));

                return .{ .bezier_curve = .{ .bezier_curve = curve.TimeCurve.init_from_linear_curve(projected_curve[0]) } };
            },
            .bezier_curve => |bez| .{ .bezier_curve = .{ .bezier_curve = curve.TimeCurve.init_from_linear_curve(self.bezier_curve.project_curve(bez.bezier_curve)[0]) } },
...
pub const TimeTopology = union(enum) {
    ...
    pub fn init_linear_start_end(start: curve.ControlPoint, end: curve.ControlPoint) TimeTopology {
        const slope = (end.value - start.value) / (end.time - start.time);
        const offset = start.value - start.time * slope;
        return init_affine(.{ .bounds = .{ .start_seconds = start.time, .end_seconds = end.time }, .transform = .{
            .scale = slope,
    ...
    pub fn init_step_mapping(
        ...
        while (t_seconds < in_bounds.end_seconds) {
            try segments.append(curve.create_linear_segment(.{ .time = t_seconds, .value = current_value }, .{ .time = t_seconds + held_duration, .value = current_value }));
    ...
    pub fn project_topology(self: @This(), other: TimeTopology) !TimeTopology {
        ...
        std.debug.print("self_tag: {any} other_tag: {any}\n", .{ self_tag, other_tag });
...
test "TimeTopology: finite identity test" {
    ...
    for (times, 0..) |t, index| {
        ...
        errdefer std.log.err("[{d}] time: {d} expected: {d} err: {any}", .{ index, t, expected_result[index], err[index] });
        ...
        if (err[index]) {
            ...
            try expectApproxEqAbs((try tp.project_sample(s)).ordinate_seconds, expected_result[index], util.EPSILON);

            try expectApproxEqAbs(try tp.project_ordinate(t), expected_result[index], util.EPSILON);
        }
    }
}
...
test "TimeTopology: finite Affine" {
    ...
    for (tests, 0..) |t, index| {
        errdefer std.log.err("[{d}] time: {d} expected: {d} err: {any}", .{ index, t.seconds, t.expected, t.err });
        ...
        if (t.err) {
            ...
            try expectApproxEqAbs(t.expected, try tp.project_ordinate(t.seconds), util.EPSILON);
        }
    }
}
...
test "TimeTopology: Affine Projected Through inverted Affine" {
    ...
    const expected_bounds = interval.ContinuousTimeInterval{ .start_seconds = 10, .end_seconds = 30 };
    try expectApproxEqAbs(expected_bounds.start_seconds, tp_inv.bounds().start_seconds, util.EPSILON);
    try expectApproxEqAbs(expected_bounds.end_seconds, tp_inv.bounds().end_seconds, util.EPSILON);
...
test "TimeTopology: Affine Projected Through infinite Affine" {
    ...
    const expected_bounds = interval.ContinuousTimeInterval{ .start_seconds = 0, .end_seconds = 10 };

    try expectApproxEqAbs(expected_bounds.start_seconds, tp_through_tp_inf.bounds().start_seconds, util.EPSILON);
    try expectApproxEqAbs(expected_bounds.start_seconds, tp_inf_through_tp.bounds().start_seconds, util.EPSILON);

    try expectApproxEqAbs(expected_bounds.end_seconds, tp_through_tp_inf.bounds().end_seconds, util.EPSILON);
    try expectApproxEqAbs(expected_bounds.end_seconds, tp_inf_through_tp.bounds().end_seconds, util.EPSILON);

    // projecting ordinates through both should result in the original argument
    var time: Ordinate = expected_bounds.start_seconds;
    const end_point = expected_bounds.end_seconds;
    while (time < end_point) : (time += 0.1) {
        errdefer std.log.err("time: {any}\n", .{time});
        ...
        errdefer std.log.err("time: {any} result_tp: {any} result_inf: {any}\n", .{ time, result_tp, result_inf });
...
test "TimeTopology: Affine through Affine w/ negative scale" {
    ...
    const output_to_media = try intrinsic_to_media.project_topology(output_to_intrinsic);
    ...
    for (output_to_media_tests, 0..) |t, index| {
        errdefer std.log.err("[{d}] time: {d} expected: {d} err: {any}", .{ index, t.output_s, t.media_s, t.err });
        ...
        if (t.err) {
            ...
            try expectApproxEqAbs(t.media_s, try output_to_media.project_ordinate(t.output_s), util.EPSILON);
        }
    }
}
...
test "TimeTopology: staircase constructor" {
    ...
    const tp = try TimeTopology.init_step_mapping(.{ .start_seconds = 10, .end_seconds = 20 }, 0, 2, increment);

    try expectEqual(@as(usize, 5), tp.bezier_curve.bezier_curve.segments.len);
    ...
    try expectEqual(@as(f32, 20), tp.bounds().end_seconds);
...
test "TimeTopology: project affine through bezier" {
    const aff_transform = (transform.AffineTransform1D{
        .offset_seconds = 100,
        .scale = 1,
    });
    const aff_bounds = .{
    ...
    const aff_Topo = (TimeTopology.init_affine(.{
