...
//!
//! Reads .otio (JSON), .ziggy, or .tlb (binary) files and writes to
//! .ziggy or .tlb format based on the output file extension.
//!
//! Usage:
//!   otiocat <input> <output>
//!
//! Examples:
//!   otiocat timeline.otio timeline.ziggy   # JSON to Ziggy
//!   otiocat timeline.otio timeline.tlb     # JSON to Binary
...
) noreturn
{
    std.debug.print(
        ...
        \\  <output>  Path for the converted output file
        \\
        \\Options:
...
pub fn main() !void
{
    ...
    const output_ext = get_extension(state.output_path) orelse {
        std.log.err("Output file must have an extension (.ziggy or .tlb)", .{});
        std.process.exit(1);
    };

    if (!std.mem.eql(u8, output_ext, ".ziggy") and
        !std.mem.eql(u8, output_ext, ".tlb"))
    {
        std.log.err(
            "Unsupported output format: {s}. Use .ziggy or .tlb",
            .{output_ext}
        );
        std.process.exit(1);
    }
    ...
    // Open output file
    var out_file = try std.fs.cwd().createFile(state.output_path, .{});
    ...
    {
        std.log.err(
            "Unsupported input format: {s}. Use .otio, .ziggy, or .tlb",
            .{input_ext}
        );
        std.process.exit(1);
    }
    ...
    std.log.info("Wrote: {s}", .{state.output_path});
}
