...
const pos3d_world = [3]f32;
const pos2d_screen = [2]f32;
const color_index = i8;

const Uniforms = extern struct {
    planet_radius: f32 = 1,
    iResolution: [2]f32 = [2]f32{1, 1},
    invResolution: [2]f32 = [2]f32{1,1},

    theta: f32 = 0,
    cloud_theta: f32 = 0,
    tx_zoom: f32 = 1,

    light_position: pos3d_world = pos3d_world{3,3,3},
    planet_position: pos3d_world = pos3d_world{0.5, 0, 0},

    iTime: f32 = 0,
    rotation_rate: f32 = 3.5,
    light_samples: i16 = 4,

    water_height: i32 = 100,
    water_color_index: color_index = 6,

    desert_height: i32 = 18,
    desert_color_index: color_index = 1,

    forest_height: i32=37,
    forest_color_index: color_index = 3,

    mountain_height: i32=64,
    mountain_color_index: color_index = 0,

    // moon stuff
    moon_position: pos3d_world = pos3d_world{0.6, 0.6, 0.6},
    moon_radius:f32=0.15,
    moon_phi:f32=0.0,
    moon_orbit_rate:f32=0.0,
    moon_orbit_radius:f32=1.0,
...
fn _menu_rendered(demo:*DemoState) void {
    if (!zgui.begin("Demo Settings", .{})) {
        zgui.end();
        return;
    }

    if (zgui.collapsingHeader("Texture Generation", .{})) {
        zgui.sliderInt(
            "Iterations",
            .{ 
                .v = &tex_synth_params,
        ...
        if (zgui.smallButton("regenerate texture")) {
            // for now crash
            unreachable;
        }
    }
