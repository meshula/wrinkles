...
) !State 
{
    ...
    var output_json_fpath:[]const u8 = undefined;
        ...
        |nextarg| 
    {
        ...
        switch (arg_count) {
            ...
            2 => {
                output_json_fpath = try allocator.dupe(u8, fpath);
            },
            else => {
...
) void 
{
    std.debug.print(
        \\
        \\Parse the .otio file into the wrinkles and serialize it as json
        \\
        \\usage:
        \\  otio_dump_json path/to/somefile.otio path/to/output.json
        \\
        \\arguments:
